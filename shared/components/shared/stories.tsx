	...
	return (
		<>
			<Container className={cn("sm:w-[100px] md:w-full sm:flex sm:items-center sm:justify-between sm:gap-1 md:gap-2 md:my-10", className)}>
				{stories.length === 0 &&
					[...Array(6)].map((_, index) => (
						<div key={index} className="md:w-[200px] md:h-[250px] sm:hidden md:block bg-gray-200 rounded-md animate-pulse" />
					))}
				...
				{stories.map((story) => (
					<img
						...
						className="rounded-md cursor-pointer md:w-[200px] md:h-[250px] sm:w-[60px] sm:h-[60px] "
